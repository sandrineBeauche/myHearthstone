...
public class
SunburstChart<T extends ChartItem> extends Region {
    ...
    public SunburstChart(final TreeNode<T> TREE) {
        ...
        treeNodeListener       = EVENT -> {
            if(EVENT.Type()!= TreeNodeEventType.NODE_SELECTED){
                redraw();
            }
        };
        initGraphics();
    ...
    private void drawChart() {
        HashMap<Integer, ArrayList<TreeNode<T>>> levelMap = this.tree.getLevelMap();
        int maxLevel = this.tree.getMaxLevel();
        ...
        for (int level = 1 ; level <= maxLevel ; level++) {
            ...
            for (TreeNode<T> node : nodesAtLevel) {
                ...
                double    segmentAngle;
                Paint     segmentColor = getUseColorFromParent() ? node.getTreeRoot().getItem().getFill() : segmentData.getFill();
